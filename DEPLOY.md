...
2. `/mybots` → ваш бот → **Bot Settings → Inline Mode → Turn on**. Це потрібно для кнопки «Share with a friend» усередині гри — вона працює через `switchInlineQuery`, офіційний спосіб Telegram для поширення з Mini App із поверненням користувача назад у застосунок (на відміну від звичайного відкриття посилання, яке просто закриває гру).
3. Більше нічого налаштовувати в BotFather не треба — на відміну від класичних Games тут не потрібні `/newgame` чи short name. Постійна кнопка меню («Play») бот виставляє собі сам автоматично при старті (`setChatMenuButton` у `server.js`).
...
На екрані «Mission failed» два способи відродитись із одним щитом, по черзі:

1. **Перший провал** — «📤 Share with a friend · +1 shield». Викликає `tg.switchInlineQuery()` — офіційний Telegram-метод саме для поширення з середини Mini App: відкривається звичайний вибір чату (inline-режим), а після вибору й надсилання Telegram **автоматично повертає гравця назад у гру** (на відміну і від класичних Games, і від звичайного `openTelegramLink`, які просто закривають застосунок). Бекенд відповідає на inline-запит через `answerInlineQuery` (див. `server.js`). Оскільки Telegram не дає callback «повідомлення реально надіслано», нагорода видається за стандартною евристикою: коли гравець повертається у застосунок.
2. **2-4 провал** — «▶ Watch ad · +1 shield» через [Adsgram](https://adsgram.ai), до 3 разів.
3. Далі — гра завершується остаточно.

У `game/index.html` вкажіть посилання на бота:
```js
const SHARE_URL = 'https://t.me/YOUR_BOT_USERNAME';
```

### Реклама (Adsgram)
Поки `AD_BLOCK_ID` не налаштований — показується 3-секундна симуляція для тестування. Щоб увімкнути реальну рекламу: зареєструйтесь на [adsgram.ai](https://adsgram.ai), створіть Rewarded-блок, вставте Block ID у:
```js
...
4. Пройдіть раунд, програйте, спробуйте «Share with a friend» — має відкритись нативне вікно поширення Telegram, гра не повинна закритись.
5. Рахунок автоматично піде на `/api/score`, перевіряється через офіційну `initData`-верифікацію.
6. Переглянути топ гравців: `GET https://ptn-pnh-bot.onrender.com/api/leaderboard` (простий JSON, без нативного інтерфейсу — Mini App не має вбудованого лідерборду, як мали класичні Games).
